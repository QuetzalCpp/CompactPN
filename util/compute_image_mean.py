...
# =========== Resize images to train and save the mean ============================
...
for i, impath in enumerate(imlist):
    ...
    print('[%d/%d]:%s' % (i+1, len(imlist), impath))
...
# =========== Resize images to test ============================
# save resized test images
if args.save_resized_imgs:
    imlist =  np.loadtxt(jpath(dataroot, 'dataset_scene2_validation2_test.txt'),
    #imlist =  np.loadtxt(jpath(dataroot, 'scene1_validation1_test.txt'),
    ...
    """imlist =  np.loadtxt(jpath(dataroot, 'scene1_validation2_test.txt'),
                        dtype=str, delimiter=' ', skiprows=3, usecols=(0))
    for i, impath in enumerate(imlist):
        print('[%d/%d]:%s' % (i+1, len(imlist), impath))
        #print('[%d/%d]:%s' % (i+1, len(imlist), impath), end='\r')
    ...
    imlist =  np.loadtxt(jpath(dataroot, 'scene1_validation3_test.txt'),
                        dtype=str, delimiter=' ', skiprows=3, usecols=(0))
    for i, impath in enumerate(imlist):
        print('[%d/%d]:%s' % (i+1, len(imlist), impath))
        #print('[%d/%d]:%s' % (i+1, len(imlist), impath), end='\r')
